...
async def create_article(article: CategoryArticleCreateSchema, db: Session = Depends(get_db), authorization: str = Header(...)): 
...
async def read_category_articles(db: Session = Depends(get_db), authorization: str = Header(...)):
    print(authorization)
    check_valid_token(authorization)
...
async def read_category_article(category_article_id: int, db: Session = Depends(get_db), authorization: str = Header(...)):
    check_valid_token(authorization)
...
async def patch_article(article_id: int, article: CategoryArticleUpdateSchema, db: Session = Depends(get_db), authorization: str = Header(...)):
    check_valid_token(authorization)
...
async def delete_article(category_article_id: int, db: Session = Depends(get_db), authorization: str = Header(...)):
    check_valid_token(authorization)
